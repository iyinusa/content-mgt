...
class Category extends CI_Controller {
	...
	public function index() {
		...
		if($this->session->userdata('itc_user_role') != 'Admin'){ 
			redirect(base_url().'documents/', 'location');
		}
		
		//check for update
		$get_id = $this->input->get('edit');
		if($get_id != ''){
			$gq = $this->user->query_rec_single('id', $get_id, 'bz_category');
			foreach($gq as $item){
				$data['e_id'] = $item->id;
		...
		if($del_id != ''){
			if($this->user->delete_rec('id', $del_id, 'bz_category') > 0){
		...
		if($_POST){
			...
			if($cat_id != ''){
				...
				if($this->user->update_rec('id', $cat_id, 'bz_category', $upd_data) > 0){
				...
				if($this->user->reg_rec('bz_category', $reg_data) > 0){
					$data['err_msg'] = '<div class="alert alert-info"><h5>Successfully</h5></div>';
				} else {
					$data['err_msg'] = '<div class="alert alert-info"><h5>There is problem this time. Try later</h5></div>';
				}
		...
		//query uploads
		$data['allup'] = $this->user->query_rec('bz_category');
